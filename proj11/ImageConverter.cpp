...
//		invertImage(); // from lab
	//	invertImage2(); // from lab
	//	invertImage3(); // from lab
	rotateRightAndFlipVertical();
//	grayScale(); // not counted
	myCanvas0.wait();
}	
...
/*
 * cropImage() crops a certain size from the original image.
 * Uses the user input x, y, width, and height to crop.
 * Takes the users inputed x and y value to set the new origin at new canvas.
 * Then takes the new width (max 450) and new height (max 400) to crop.
 * Draws the image at the new canvas by getting the pixel from original from x, y to the user width and user height
 * Crops from bottom left to top right.
 */
void ImageConverter::cropImage(unsigned x, unsigned y, unsigned width, unsigned height) {
	// record starting time
	double startTime = omp_get_wtime();

	Canvas cropped(myWidth+50, 0, myWidth, myHeight,
			myImageFileName+"-cropped");
	cropped.start();

	Background * background0 = myCanvas0.getBackground();
	Background * croppedImage = cropped.getBackground();


	const int START_X  = x,            // x origin
			START_Y    = y,            // y origin
			STOP_X     = width,   // goes to the right from origin
			STOP_Y     = height;  // goes to the top from origin

	// loop through the image rows
#pragma omp parallel for
	for (int y = START_Y; y < STOP_Y; ++y) {          // origin to top
		// loop through the image columns
		for (int x = START_X; x < STOP_X; ++x) {      // origin to right
			// read the pixel at canvas0[row][col]
			ColorInt pixelColor = background0->getPixel(x, y);

			// gives the same color
			ColorInt Color(pixelColor.R, pixelColor.G, pixelColor.B);

			// draw the cropped image
			croppedImage->drawPixel(x, y, Color);
		} // inner for
		// slow the processing to simulate a very large image
		cropped.sleep();
	} // outer for
	...
	//save crop image
//	cropped.takeScreenShot();

	cropped.wait();
}

/*
 * rotateRightAndFlipVertical() rotates the image to the right and flip the image vertically.
 * Gets the pixel at position x, y from the original canvas.
 * Then draws the pixel into the new canvas at position y, x by switching the x and y position.
 */

void ImageConverter::rotateRightAndFlipVertical() {
	// record starting time
	double startTime = omp_get_wtime();

	Canvas canvas1(myWidth+50, 0, myWidth, myHeight,
			myImageFileName+"-rotateRightAndFlipVertical");
	canvas1.start();
	Background * background0 = myCanvas0.getBackground();
	Background * background1 = canvas1.getBackground();
...
#pragma omp parallel for
	for (int y = START_Y; y < STOP_Y; ++y) {          // bottom to top
		...
		for (int x = START_X; x < STOP_X; ++x) {      // left to right
			...
			ColorInt pixelColor = background0->getPixel(x, y);
			ColorInt color(pixelColor.R, pixelColor.G, pixelColor.B);
			// draw rotated right and flip vertical in canvas1
			background1->drawPixel(y, x, color);
		} // inner for
		// slow the processing to simulate a very large image
		canvas1.sleep();
	} // outer for
	...
	// compute and display the time difference
	double totalTime = omp_get_wtime() - startTime;
	cout << "\n\nImage rotated right and flip vertical took "
			<< totalTime << " seconds.\n" << endl;
...
/*
 * not counted towards project 11.
 */
...
#pragma omp parallel for
	for (int y = START_Y; y < STOP_Y; ++y) {          // bottom to top
		...
		for (int x = START_X; x < STOP_X; ++x) {      // left to right
			...
			int invertedG = 255 - pixelColor.G;
			int invertedB = 255 - pixelColor.B;
			int average = (invertedR + invertedG + invertedB) / 3;
			ColorInt invertedColor(average, average, average);
			// draw the inverse at the same spot in canvas1
			background1->drawPixel(x, y, invertedColor);
		} // inner for
		// slow the processing to simulate a very large image
...
//flip/mirror, change background/make collage

...
void ArrayQueueTester::testPalindrome() {
	cout << "testing palindrome..." << flush;

	// direct test
	string input = "input.txt";
	string output = "output.txt";
	palindromeDetector p1(input, output);

	assert(p1.isPalindrome("level") == true);
	assert(p1.isPalindrome("firetruck") == false);
	assert(p1.isPalindrome("Madam, I'm Adam.") == true);
	assert(p1.isPalindrome("A man, a plan, a canal, Panama!") == true);

	// testing output to file
	p1.detectPalindromes();

	// testing grader's file
	string input_1 = "grader.txt";
	string output_1 = "grader_output.txt";
	palindromeDetector p2(input_1, output_1);

	p2.detectPalindromes();
